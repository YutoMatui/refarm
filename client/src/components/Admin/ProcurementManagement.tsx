...
export default function ProcurementManagement() {
    ...
    const filteredAggregation = useMemo(() => {
        ...
        return aggregationData.filter(farmer => {
            ...
            // If filterText exists, filter products as well
            if (filterText) {
                return {
                    ...farmer,
            ...
            return farmer;
        }).filter(farmer => farmer.products.length > 0); // Remove farmers with no products after filtering
    ...
    return (
        <div className="bg-white rounded-lg shadow">
            <div className="p-6 border-b flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h2 className="text-xl font-bold">仕入れ集計（配送日別）</h2>
                    <p className="text-sm text-gray-600 mt-1">指定日の注文から必要な仕入れ数量を集計します</p>
                </div>
                <button
            ...
            <div className="overflow-x-auto">
                        ...
                        {filteredAggregation.map((farmer) => (
                            <>
                                {farmer.products.map((product, idx) => (
                                    <tr key={`${farmer.farmer_id}-${product.product_name}-${idx}`} className="hover:bg-gray-50">
                                        {idx === 0 && (
                                            <td 
                                                className="px-6 py-4 text-sm font-bold text-gray-900 bg-gray-50" 
                                                rowSpan={farmer.products.length}
                                            >
                                                {farmer.farmer_name}
                                                <div className="text-xs text-gray-500 font-normal">ID: {farmer.farmer_id}</div>
                                            </td>
                                        )}
                ...
                {filteredAggregation.length === 0 && (
                    <div className="text-center py-12 text-gray-500">
                        <p className="text-lg font-medium">指定日の注文がありません</p>

...
class FarmerBase(BaseModel):
    ...
    cover_photo_url: Optional[str] = Field(None, max_length=500, description="背景（カバー）画像URL")
    commitments: Optional[list[Any]] = Field(default_factory=list, description="こだわり情報 (JSON)")
    achievements: Optional[list[Any]] = Field(default_factory=list, description="実績リスト (JSON)")
    chef_comments: Optional[list[Any]] = Field(default_factory=list, description="シェフからのコメント (JSON)")
...
class FarmerUpdate(BaseModel):
    ...
    commitments: Optional[list[Any]] = Field(None)
    achievements: Optional[list[Any]] = Field(None)
    chef_comments: Optional[list[Any]] = Field(None)
